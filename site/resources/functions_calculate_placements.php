...
function replace_id_with_name($placements, $students_by_id)
{
	...
	foreach($placements as &$this_placement)
	{
		usort($this_placement->students_alloc, "sort_random");
		unset($this_placement);
	}
...
function upload_report($report_output, $this_iteration_output, $report_file)
{
	...
	{
		print  date('d.m.Y-H:i:s:', time()) . ' Error writing report.<br />';
	}
}
...
function variable_mode_randomify_places($current_placement)
{
	if($current_placement->essential === true)
		// change essentials
	{
		if(rand(0, 100) >= (100 - get_VARIABLE_MODE_CHANGE_ESSENTIAL_PERC()))
		{
			$current_placement->places_max = round(rand(intval($current_placement->places_min), intval($current_placement->places_max)));
		}
	}
	else
	{
		...
		{
			if(rand(0, 100) >= (100 - get_VARIABLE_MODE_CHANGE_REMOVE_NON_ESSENTIAL_PERC()))
			{
				$current_placement->places_max = round(rand(intval($current_placement->places_min), intval($current_placement->places_max)));
			}
		}
		...
		{
			$current_placement->places_max = 0;
			$current_placement->places_min = 0;
		}
	}
				...
				elseif(!(empty($current_student_array)) && !($places_target == 0) && ($places_target >= count($current_student_array)) && (!($key == (count($priority_types) - 2)) || ($places_target == count($current_student_array))) && (!($key == (count($priority_types) - 1)) || ($current_placement->places_min <= (count($current_placement->students_alloc) + count($current_student_array))) || ($current_placement->places_min <= count($current_student_array))))
				{
					...
					foreach($current_student_array as $current_student)
					{
						...
						if($key == 0)
						{
							...
							$iteration_output[$iterations] .= ' <i>. He used a Joker in this round. So his Joker will be subtracted. Goodbye, Joker!;</i> ';
						}
						else
						{
							if(in_array($key, get_PRIORITIES_AFFECTING_KARMA()) && !(count($deployment_placements[$current_placement->deployment]->placements) == 1))
					...
					for($i = 0; $places_target > $i; $i++)
					{
						...
						foreach($current_student_array as $current_student)
						{
							if($current_student_array[$i]->id == $current_student->id)
							{
								...
								{
									$current_student->joker = ($current_student->joker - 1);
									$iteration_output[$iterations] .= ' (used a Joker in this round. So his Joker will be subtracted)';
								}
								else
								{
									// Reduce karma points							
...
{
	$calc_all_allocated = false;
	foreach($multiplied_iteration as $iteration)
	{
		if(empty($iteration->unallocated_students))
		{
			$calc_all_allocated = true;
