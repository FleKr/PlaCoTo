...
if(check_for_placement_validity($_GET["id"], $this_student->GROUP))
{
	...
	if(!($user_wishes === false))
	{
		if(isset($_GET["submit"]) && isset($_POST["valid_request"]))
		{
			...
			foreach($deployments as $deployment)
			{
				...
				if(isset($_POST['deploy_' . $deployment_rm_whitespace]))
				{
					...
					if(isset($_POST[$deployment_rm_whitespace . '::joker']))
					{
						...
						if(!empty($_POST[$deployment_rm_whitespace . '::joker']))
						{
					...
					foreach($priority_types as $key => $value)
					{
						...
						if(!empty($_POST[$key . '::' . $deployment_rm_whitespace]))
						{
							$email_output .= "Your " . $value . " is " . $_POST[$key . '::' . $deployment_rm_whitespace] . "<br />";
						}
					...
					if(!empty($_POST[$location_key_num . '::' . $deployment_rm_whitespace . '::location']))
					{
						$email_output .= "Your preferred locations is " . $_POST[$location_key_num . '::' . $deployment_rm_whitespace . '::location'] . "<br />";
					}
				}
			}
			...
			{
				$module_output .= " Email could not been send.";
			}
		...
		{
			$module_output .= '<div class="launch_wishes"><a href="#" onClick="MyWindow=window.open(' . "'" . 'index.php?act=show_wishes&id=' . $_GET["id"] . "','MyWindow',width=350,height=500); return false;" . '">See total wishes</a></div>';
