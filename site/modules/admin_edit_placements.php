...
if(!isset($_GET["step"]))
{
	...
	if(!empty($placements))
	{
		...
		{
			if($placement->ESSENTIAL === true)
			{
				$this_essential = " checked=\"checked\"";
			}
			else
			{
				$this_essential = "";
...
elseif($_GET["step"] == "submit")
{
	...
	{
		check_array_special_chars(array(
			$_POST[$input_count . "::id"],
			$_POST[$input_count . "::name"],
			$_POST[$input_count . "::deployment"],
			$_POST[$input_count . "::location"],
			$_POST[$input_count . "::timeframe_begin"],
			$_POST[$input_count . "::timeframe_end"],
			$_POST[$input_count . "::places_min"],
			$_POST[$input_count . "::places_max"],
		)) or die ("CRITICAL ERROR updating id " . $_POST[$input_count . "::id"]);
		$new_placements[$input_count] = new db_placements;
		$new_placements[$input_count]->ID = $_POST[$input_count . "::id"];
		if(isset($_POST[$input_count . "::essential"]))
		{
			$new_placements[$input_count]->ESSENTIAL = true;
		}
		else
		{
			$new_placements[$input_count]->ESSENTIAL = false;
		}
