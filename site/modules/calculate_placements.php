...
{
	die("can not access Placements Database.");
}
...
for($iterations = 1; $iterations < (get_ITERATIONS() + 1); $iterations++, $i_placements = array(), $i_placement_student = array())
{
		...
		foreach($i_placements as $current_placement)
		{
			...
			elseif(!(empty($current_student_array)) && !($places_target == 0) && ($places_target >= count($current_student_array)))
			{
				...
				foreach($current_student_array as $current_student)
				{
					...
					if($key == 0)
					{
						$current_student->joker = ($current_student->joker - 1);
						$report_output .= ' <i>. He used a Joker in this round. So his Joker will be subtracted. Goodbye, Joker!</i>';
					}
					$report_output .= ';';
				}
			}
				...
				for($i = 0; $places_target > $i; $i++)
				{
					...
					foreach($current_student_array as $current_student)
					{
						if($current_student_array[$i]->id == $current_student->id)
						{
							...
							{
								$current_student->joker = ($current_student->joker - 1);
								$report_output .= ' (used a Joker in this round. So his Joker will be subtracted)';
							}
							// Allocate student
							$current_placement->students_alloc[] = $current_student->id;
							$report_output .= ';</i>';
	...
	{
		$report_output .= $i_missing_placements_output;
	}
	...
	{
		$report_output .= $i_missing_min_places_output;
	}
...
foreach($iteration_describer as $iteration)
{
	if(empty($iteration->unallocated_students))
	{
		$calc_all_allocated = true;
	}
}
...
if($calc_all_allocated)
{
	...
	{
		if(!(empty($value->unallocated_students)))
		{
			$report_output .= $value->id . '; ';
			unset($iteration_describer[$iteration]);
		}
	}
}
else
{
	$report_output .= "<b>HOOOMANZ!</b> I haven't been able to calculate a table, where all students are allocated. :(";
}
...
foreach($iteration_describer as $iteration)
{
	if(empty($iteration->unallocated_min_places))
	{
		$calc_min_places_allocated = true;
	}
}
// if there is such a calculation, remove all others
if($calc_min_places_allocated)
{
	...
	foreach($iteration_describer as $iteration => $value)
	{
		if(!(empty($value->unallocated_min_places)))
		{
			$report_output .= $value->id . '; ';
			unset($iteration_describer[$iteration]);
		}
...
{
	$report_output .= "<b>HOOOMANZ!</b> I haven`t been able to calculate a table, where all minimum places are allocated. :(";
}
...
foreach($iteration_describer[0]->placements as $this_placement)
{
	...
	{
		$this_student = $students_by_id[$this_student]["NAME"];
		unset($this_student);
	}
}
...
{
	$report_output .= "<br /><b>HOOOMANZ!</b> I haven`t been able to upload the placement file. :(";
}
...
if($calc_min_places_allocated && $calc_all_allocated)
{
	...
	{
		$report_output .= "<br /><b>HOOOMANZ!</b> I haven`t been able to replace the students file. :(";
	}
...
else
{
	...
	{
		$report_output .= "<br /><b>HOOOMANZ!</b> I haven`t been able to upload the students file. :(";
	}
}
